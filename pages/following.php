...
<body>
    ...
    <div class="container" style="margin-top : 20px">
        <h2 style="text-align:center; margin-bottom : 20px">Following</h2>
        <?php if ($reqinfo_result && mysqli_num_rows($reqinfo_result) > 0) { ?>
            <?php while ($row = mysqli_fetch_assoc($reqinfo_result)) { ?>
                <div class="follower">
                    ...
                    <a class="unfollow-btn unfollow-profile-btn" data-user-id='<?php echo $row['id']  ?>'>Unfollow</a>
                    <?php } ?>
                </div>
            <?php } ?>
        <?php } else { ?>
            <p>No followeing yet.</p>
        <?php } ?>
    </div>
